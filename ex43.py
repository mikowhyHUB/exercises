...
def find_anagrams(word, candidates):
    ...
    for char in candidates:
        if word.upper() == char.upper():
            continue
        elif sorted(char.upper()) == sorted(word.upper()):
            outcome += [char]
    return outcome
    # return [char for char in candidates if word == char elif sorted(char.upper()) == sorted(word.upper())]
